...
PHOTO_K = 18              # СИЛЬНЫЙ фактор: вид/ремонт по фото (vision), ±18 при q=5/1
...
def score(x: Listing, c: Criteria) -> float:
    ...
    if any(k in text for k in _NEEDS_WORK):
        ...
        s += RENO_BONUS
    # вид/ремонт по фото (vision): q 1..5 → ±PHOTO_K. Сильный фактор.
    if x.photo_q is not None:
        s += PHOTO_K * (x.photo_q - 3) / 2.0
    return round(s, 1)
...
def score_listings(uniq: list[Listing], c: Criteria) -> list[dict]:
    """Скоринг уже дедуплицированного списка (фото-вклад берётся из x.photo_q)."""
    scored = [{"listing": x, "score": (sc := score(x, c)), "tier": tier(sc)}
              for x in uniq]
...
def rank_all(listings: list[Listing], c: Criteria) -> list[dict]:
    ...
    return score_listings(dedup(listings), c)
